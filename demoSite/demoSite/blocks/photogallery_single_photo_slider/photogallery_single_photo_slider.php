<?php
class photogallery_single_photo_slider_block_handler extends  block_handler{
    function info()
    {
        $info['category_name'] = "Photo Gallery";
        $info['category_icon'] = "dsf";

        $info['block_name'] = "Photo Slider";
    ...
    public function generate_content()
    {
		...
		$id = $segments[$count-1];
        $vid = new PhotoGalleryMedia();
		$vid = $vid->where('id',$id)->get();
		$author = new User();
		$author = $author->where('id',$vid->user_id)->get();
		
		$output ='  
		<div id="photogallery-single-photo-slider-'.$this->block->get_id().'">
		<hr />
        <!-- Work Next Prev Bar -->
                <div class="item-nav">';	
						...
						$usr = (!empty($user_galleries))?base_url('photogallery/channel/'.$user_galleries[0].''):base_url('photogallery/channel/'.$author->username.'');
                    $output .='<a class="item-prev" href="'.$usr.'">
                        <div class="prev-btn"><i class="fa fa-angle-left"></i></div>
                        <div class="item-prev-text xs-hidden">
                            <h6>Previous Gallery</h6>
                        </div>
                    </a>
                    <a href="'.base_url('photogallery/channel/'.$author->username.'').'" class="item-all-view">
                        <h4>'.$author->first_name.' '.$author->last_name.' Member Gallery</h4>
                    </a>';
					$usr= (!empty($user_galleries))?base_url('photogallery/channel/'.end($user_galleries).''):base_url('photogallery/channel/'.$author->username.'');
                    $output .='<a class="item-next" href="'.$usr.'">
                        <div class="next-btn"><i class="fa fa-angle-right"></i></div>
                        <div class="item-next-text xs-hidden">
                            <h6>Next Member Gallery</h6>
                        </div>
                    </a>
                </div>
		...
		if((isset($_SESSION['editoR']) && $_SESSION['editoR']))
			return $output.$this->apply_custom_css().$CI->layout_system->load_new_block_scripts($this->block->get_id(), 'photogallery-single-photo-slider-'.$this->block->get_id(), $CI->BuilderEngine->get_page_path(), '', $this->block->get_name(), $menu);
		else

<?php
class ecommerce_category_pagination_block_handler extends  block_handler{
    function info()
    {
        $info['category_name'] = "Online Store";
        $info['category_icon'] = "dsf";

        $info['block_name'] = "Category pagination";
...
//			$style_arr['display'] = 'flex';
    ...
    public function apply_custom_css()
    {
		...
		div[name="'.$this->block->get_name().'"] i{
                color: '.$style_arr['color'].' !important;
        }
        </style>';
    }
    public function generate_content()
    {
        ...
        if(isset($content['output']) && $content['output'] != 'default')
        {
            ...
            $total_pages = $CI->ecommerce->get_total_pages($category_name);
            $pagination = $CI->ecommerce->get_pagination($category_name, $total_pages);
        }
        else if (strpos($_SERVER['REQUEST_URI'],'search') !== false)
        {
            ...
            $total_pages = $CI->ecommerce->get_total_pages($keyword, true);
            $pagination = $CI->ecommerce->get_pagination($keyword, $total_pages, true);
        }
...
//                $category_name = 'all';
        ...
        else
        {
            ...
            if(strpos($current_url,'category=') !== false || strpos($current_url,'.html') !== false)
            {
				$_GET['category'] = 'Watches';
                $total_pages = $CI->ecommerce->get_total_pages('all');
                $pagination = $CI->ecommerce->get_pagination('Watches', $total_pages, false, true);
            }else{
				$_GET['category'] = 'Watches';
                $total_pages = $CI->ecommerce->get_total_pages('all');
                $pagination = $CI->ecommerce->get_pagination('Watches', $total_pages, false, true);

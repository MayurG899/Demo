<?php
class photogallery_myfeed_sidebar_block_handler extends  block_handler{
    function info()
    {
    ...
    public function generate_content()
    {
        ...
        $output ='
			<div id="photogallery-myfeed-sidebar-'.$this->block->get_id().'">
							...
							$output .='<div class="photogallery-sidebar-widget">
								...
								<ul>';
									foreach($albums as $album){
										$output .='<li><a href="'.base_url('photogallery/channel/'.$user->username.'/album/'.$album->id.'').'">'.str_replace('_',' ',$album->name).''.$album = ($album->status == 'private')?' - (Private)':''.'</a></li>';
									}
								$output .='</ul>
							</div>';
						}
						if($photos->tags != '' && $show_tags == 'yes'){
							$output .='<div class="photogallery-sidebar-widget">
								<p>My Photo Tags</p>
								...
								foreach($photos as $photo){
									...
									foreach($tags as $tag){
										if($i <= $num_tags){
											$output .='<li><a href="'.base_url('photogallery/photo/'.$photo->id.'').'">'.$tag.'</a></li>';
											$i++;
										}
									}
								}
		...
		if((isset($_SESSION['editoR']) && $_SESSION['editoR']))
			return $output.$this->apply_custom_css().$CI->layout_system->load_new_block_scripts($this->block->get_id(), 'photogallery-myfeed-sidebar-'.$this->block->get_id(), $CI->BuilderEngine->get_page_path(), '', $this->block->get_name(), $menu);
		else

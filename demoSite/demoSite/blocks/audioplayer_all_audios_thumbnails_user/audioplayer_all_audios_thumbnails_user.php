<?php
class Audioplayer_all_audios_thumbnails_user_block_handler extends  block_handler{
    function info()
    {
        $info['category_name'] = "Audio Player";
        $info['category_icon'] = "dsf";

        $info['block_name'] = "All User Audio Thumbnails";
    ...
    public function generate_content()
    {
		...
		$author = new User();
		$author = $author->where('username',$segments[$count-2])->get();
        ...
        $output ='<div class="row">
					...
					foreach($sounds as $sound){
						...
						if($sound_album->status != 'private' || ($sound_album->status == 'private' && $user->get_id() == $sound_album->user_id)){
							$output .='
							<div class="col-lg-4 col-md-4 col-sm-6 col-xs-12 audiogallery-recent-paddings">

<?php
class Booking_services_application_images_block_handler extends  block_handler{
	function info()
	{
		$info['category_name'] = "Booking Services";
		$info['category_icon'] = "dsf";

		$info['block_name'] = "Service Application Images";
	...
	public function generate_content()
	{
		...
		if(strpos($_SERVER['REQUEST_URI_PATH'],'booking_services/application/') !== FALSE)
		...
		$output = '
			<div id="booking-services-application-images-container-'.$this->block->get_id().'" class="booking-services-application-container module-colors">
				<div class="row">';
					if($service->exists() && ($service->additional_image->count() > 0 || $service->image != '')){
						if($service->image != ''){
							$output .='
							<div class="col-md-12">
								<img src="'.$service->image.'" class="img-thumbnail booking-services-application-images"/>
							</div>';
						}
						if($service->additional_image->count() > 0){
							foreach($service->additional_image->get() as $image){
								$output .='
								<div class="col-md-12">
									<img src="'.$image->url.'" class="img-thumbnail booking-services-application-images"/>
								</div>';
							}
						}
					}else{
		...
		if((isset($_SESSION['editoR']) && $_SESSION['editoR']))
			return $output.$this->apply_custom_css().$CI->layout_system->load_new_block_scripts($this->block->get_id(), 'booking-services-application-images-container--'.$this->block->get_id(), $CI->BuilderEngine->get_page_path(), $single_element, $this->block->get_name(), $menu);
		else

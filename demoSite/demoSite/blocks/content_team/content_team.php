<?php
class content_team_block_handler extends  block_handler{
	function info()
	{
		$info['category_name'] = "Basic Blocks";
		$info['category_icon'] = "dsf";

		$info['block_name'] = "Team Members";
    ...
    {	$this->block->set_data("editorEnabled", 1);
		...
		$output = '
			<div block-editor="ckeditor" block-name="'.$this->block->get_name().'" id="team-container-'.$this->block->get_id().'" class="">
		...
		if((isset($_SESSION['editoR']) && $_SESSION['editoR']))
			return $output.$CI->layout_system->load_new_block_scripts($this->block->get_id(), 'team-container-'.$this->block->get_id(), $CI->BuilderEngine->get_page_path(), $single_element, $this->block->get_name(), $menu);
		else
			return $output;
    }
}

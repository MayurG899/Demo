<?php
class Videotube_channel_comments_block_handler extends  block_handler{
    function info()
    {
    ...
    public function generate_content()
    {
		...
		$author_settings = $CI->videotubeusersettings->where('user_id',$author->id)->get();
		...
		$num_albums = $CI->videotubealbum->where('user_id',$author->id)->count();
		$num_tags = $CI->BuilderEngine->get_option('videotube_num_tags_displayed');
		$comments = $CI->videotubecomment->where('channel_owner_id',$author->id)->get();
		$comments_count = $CI->videotubecomment->where('channel_owner_id',$author->id)->count();
		...
		<div class="clearfix"></div>';
						if($CI->BuilderEngine->get_option('videotube_comments_private')== 'public' || ($CI->BuilderEngine->get_option('videotube_comments_private')== 'private' && $user->is_logged_in())){
							$output .='<div class="videotube-comment-post-comment">
								<h4><span class="videotube-comment-comment-numb">'.$comments_count.'</span> Channel Comments</h4>
										...
										<div class="">
											<div class="videotube-comment-comment-detail">';
												...
												$output .='<div id="reportDiv'.$i.'" class="hide" style="border:1px solid #ddd;margin-top:5px;">
													...
													<form id="reportForm'.$i.'" method="get" action="'.base_url('videotube/report_comment/'.$author->username).'">

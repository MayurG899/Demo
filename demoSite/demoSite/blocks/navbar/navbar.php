			...
			*/
            ...
            $output .= '
				<div id="navbar-container-'.$this->block->get_id().'">
					<div class="navbar-header navbar navbar-inverse custom-content">
					...
					<div class="collapse navbar-collapse" id="header-navbar" style="">
						<ul class="nav navbar-nav navbar-right" id="header-navbar-'.$this->block->get_id().'">';
						...
						foreach($CI->links->get_all_links() as $link)
						{
							...
							{
								$output .= '
								<a href="'.checkLink($link->target,$link->tags).'" title="'.$link->title.'" class="dropdown-toggle text-color-'.$this->block->get_id().' text-size-'.$this->block->get_id().'" data-toggle="dropdown" style="">
									'.$link->name.'
									<b class="caret"></b>                           
								</a>
								<ul class="dropdown-menu dropdown-menu-left animated fadeIn dropdown-background-color-'.$this->block->get_id().'" style="z-index:10000">';
								foreach($children as $sub_link)
								{
									$output .= '
									<li class="dropdown-submenu dropdown-background-color-hover-'.$this->block->get_id().'">';
									$second_children = $CI->links->link_has_children($sub_link->id);
									if(is_array($second_children))
									{
										$output .= '
										<a href="'.checkLink($link->target,$link->tags).'" title="'.$sub_link->title.'" class="dropdown-toggle dropdown-color-'.$this->block->get_id().' dropdown-size-'.$this->block->get_id().'" data-toggle="dropdown" style="">
											'.$sub_link->name.'
											<i class="fa fa-caret-right pull-right" aria-hidden="true"></i>                           
										</a>
										<ul class="dropdown-menu dropdown-menu-left animated fadeIn dropdown-background-color-'.$this->block->get_id().'" style="z-index:10000">';
										foreach($second_children as $subsub_link){
											$output .= '<li><a href="'.checkLink($subsub_link->target,$subsub_link->tags).'" class="dropdown-color-'.$this->block->get_id().' dropdown-size-'.$this->block->get_id().'" title="'.$subsub_link->title.'" style="">'.$subsub_link->name.'</a></li>';
										}
										$output .=
										'</ul>';
										$output .=
										...
										$output .= '<a href="'.checkLink($sub_link->target,$sub_link->tags).'" class="dropdown-color-'.$this->block->get_id().' dropdown-size-'.$this->block->get_id().'" title="'.$sub_link->title.'" style="">'.$sub_link->name.'</a></li>';
								}
								$output .=
								'</ul>';
							}
								...
								if(!$CI->links->is_child($link->id))
									$output .= '<a class="text-color-'.$this->block->get_id().' text-size-'.$this->block->get_id().'" href="'.checkLink($link->target,$link->tags).'" title="'.$link->title.'" style="">'.$link->name.'</a>';
							$output .= '
							</li>';
						}
						$output .= '

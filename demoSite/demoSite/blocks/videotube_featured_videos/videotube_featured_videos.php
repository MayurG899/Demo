<?php
class Videotube_featured_videos_block_handler extends  block_handler{
    function info()
    {
        $info['category_name'] = "VideoTube";
        $info['category_icon'] = "dsf";

        $info['block_name'] = "Featured Videos";
    ...
    public function generate_content()
    {
		...
		$videos = $CI->videotubemedia->where('featured','yes')->get();
		...
		//View
        $output ='
                <div id="videotube-featured-videos-container-'.$this->block->get_id().'">
				<div class="row">
				<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
					<p class="videotube-featured-title">Featured Videos</p>
				</div>';
				$i = 1;
								...
								<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
									<div class="videotube-video-item-box">';
										...
										<p class="videotube-video-text-gray"><span>'.number_format($num_video_views).' </span>Views <span> &nbsp;•&nbsp; </span> <span>Uploaded: </span>'.date('d.m.Y',$video->time_created).'</p>';
													...
													$output .='
										<p class="videotube-video-text-gray"><span>By: </span>'.$author->first_name.' '.$author->last_name.'</p>
										</div>
										</a>
									</div>
								</div>';
        ...
        if($this->block->has_class_css('header-section') == 1 || $this->block->has_class_css('footer-section') == 1)
			...
			if((isset($_SESSION['editoR']) && $_SESSION['editoR']))
				return $output.$CI->layout_system->load_new_block_scripts($this->block->get_id(), 'videotube-featured-videos-container-'.$this->block->get_id(), $CI->BuilderEngine->get_page_path(), $single_element, $this->block->get_name(), $menu);
			else
        return $output;
    }
}

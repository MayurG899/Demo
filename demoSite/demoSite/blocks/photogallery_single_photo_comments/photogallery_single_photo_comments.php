<?php
class photogallery_single_photo_comments_block_handler extends  block_handler{
    function info()
    {
        $info['category_name'] = "Photo Gallery";
        $info['category_icon'] = "dsf";

        $info['block_name'] = "Photo Comments";
    ...
    public function generate_content()
    {
		...
		$author_albums = $CI->photogalleryalbum->where('user_id',$photo->user_id)->get();
		...
		$num_albums = $CI->photogalleryalbum->where('user_id',$photo->user_id)->count();
		$num_tags = $CI->BuilderEngine->get_option('photogallery_num_tags_displayed');
		$author = new User();
		$author = $author->where('id',$photo->user_id)->get();
		
		if($allow_comments == 'yes' && $photo->comments_allowed != 'hide'){
			//View
			$output ='
			<div id="photogallery-single-photo-comments-'.$this->block->get_id().'">
			<hr /><div class="clearfix"></div>';
				if($CI->BuilderEngine->get_option('photogallery_comments_private')== 'public' || ($CI->BuilderEngine->get_option('photogallery_comments_private')== 'private' && $user->is_logged_in())){
					$output .='<div class="photogallery-comment-post-comment">
						<h4>Comments <span class="photogallery-comment-comment-numb">('.number_format($num_comments).')</span></h4>
					...
					if($allow_comments =='yes' && $photo->comments_allowed != 'no'){
								...
								<form id="postForm" method="post">
									<div>
										...
										<div class="col-lg-9 col-md-9 col-sm-12 col-xs-12">
											...
											<button id="poster" type="submit" class="btn btn-sm btn-inverse photogalleries-single-comment-button">Post Comment</button>
										</div>
									</div>
								</form>
							</div>
		...
		$(document).ready(function() {
			$(\'#like\').click(function(e){
				...
				if(permission == \'true\'){
					...
					status = \'like\';
					setTimeout(function(){
						$.get("'.base_url().'photogallery/ajax/toggle_like_photo/'.$user->id.'/'.$photo->id.'/like", function(data){
							data = $.parseJSON(data);
			...
			$(\'#unlike\').click(function(e){
				...
				if(permission == \'true\'){		
					...
					status = \'unlike\';
					setTimeout(function(){
						$.get("'.base_url().'photogallery/ajax/toggle_like_photo/'.$user->id.'/'.$photo->id.'/unlike", function(data){
							data = $.parseJSON(data);
			...
			$(\'#follow1\').click(function(e){
				...
				if(permission == \'true\'){
					...
					setTimeout(function(){
						$.get("'.base_url().'photogallery/ajax/toggle_follow/'.$user->id.'/'.$author->id.'", function(data){
							data = $.parseJSON(data);
			...
			$output = '<div id="photogallery-single-photo-comments-'.$this->block->get_id().'"></div>';
		}
		...
		if((isset($_SESSION['editoR']) && $_SESSION['editoR']))
			return $output.$this->apply_custom_css().$CI->layout_system->load_new_block_scripts($this->block->get_id(), 'photogallery-single-photo-comments-'.$this->block->get_id(), $CI->BuilderEngine->get_page_path(), '', $this->block->get_name(), $menu);
		else

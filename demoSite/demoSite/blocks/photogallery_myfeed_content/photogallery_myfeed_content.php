<?php
class photogallery_myfeed_content_block_handler extends  block_handler{
    function info()
    {
    ...
    public function generate_content()
    {
		...
		$followers = $CI->photogalleryfollow->where('follower_id',$user->id)->get();
		$followings =$CI->photogalleryfollow->where('follower_id',$user->id)->get();
		...
		$likes = $CI->photogallerylike->where('user_id',$user->id)->get();
		
		//View
        $output ='
				<div id="photogallery-myfeed-content-'.$this->block->get_id().'">
                        <div class="clearfix"></div>
						  <div class="photogallery-comment-post-comment">
                            <h4>Followers Newsfeed <span class="photogallery-comment-comment-numb"></span></h4>
                            <ul class="photogallery-comment-comment-list photogallery-comment-comment-list-myfeed mt-30">';
							foreach($followings as $following){
								$last_photos = new PhotoGalleryMedia();
								$last_photos =$last_photos->where('user_id',$following->following_id)->order_by('time_created','desc')->get();
								foreach($last_photos as $last_photo){
									$photo_author = new User($last_photo->user_id);
									$photo_album = new PhotoGalleryAlbum($last_photo->album_id);
									if($photo_album->status != 'private'){
									$output .='	<li>
											<div class="photogallery-comment-comment-avatar photogallery-myfeed-avatar">
												<a href="'.base_url('photogallery/channel/'.$photo_author->username.'').'"><img src="'.$photo_author->avatar.'"></a>
											</div>
											<div class="panel photogallery-header-dark">
												<div class="panel-body photogallery-panel-body-white">
											<div class="">
												<div class="photogallery-comment-comment-detail">
												   <span class="post-meta pull-right">Uploaded a New Photo</span> <a href="'.base_url('photogallery/channel/'.$photo_author->username.'').'"><h6>'.$photo_author->first_name.' '.$photo_author->last_name.'</h6></a>
													<div class="post-meta"><span class="photogallery-comment-font-italic">'.date('M d,Y',$last_photo->time_created).'</span></div>
													<p class="photogallery-comment-desc">'.stripslashes(ChEditorFix($last_photo->description)).'</p>
													<div class="photogallery-widget-post-media">
														<a href="'.base_url('photogallery/photo/'.$last_photo->id.'').'">
															<img alt="'.$last_photo->description.'" src="'.checkImagePath($last_photo->file).'" class="img-responsive">
														</a>
													</div>
												</div>
											</div>
		...
		if((isset($_SESSION['editoR']) && $_SESSION['editoR']))
			return $output.$this->apply_custom_css().$CI->layout_system->load_new_block_scripts($this->block->get_id(), 'photogallery-myfeed-content-'.$this->block->get_id(), $CI->BuilderEngine->get_page_path(), '', $this->block->get_name(), $menu);
		else

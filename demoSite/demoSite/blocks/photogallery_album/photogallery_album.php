<?php
class photogallery_album_block_handler extends  block_handler{
    function info()
    {
    ...
    public function generate_content()
    {
		...
		$albums = $CI->photogalleryalbum->where('user_id',$channel_owner->id)->get();
		$photos = $CI->photogallerymedia->where('album_id',$album_id)->get();
		$alb = $CI->photogalleryalbum->where('id',$album_id)->get();
		
		//View
        $output ='
		<div id="photogallery-album-'.$this->block->get_id().'">
		<!-- Work Detail Section -->
        <section class="">
            <div class="">
                <!-- work Filter -->
                <div class="row photogallery-category-albums-thumbnail-container">
							...
							foreach($albums as $album){
								if($user->get_id() == $album->user_id){
									...
									if($album->status != 'private'){
										...
										$output .='<li><a class="categories btn btn-colors btn-sm" data-filter=".'.$album->name.'">Private album</a></li>';
									}
								}
							}
            $output .='</ul>
                ...
                <!-- End work Filter -->
                <div class="container-masonry nf-col-3" style="padding-bottom:20px;">';
					foreach($photos as $photo){
					 $album = new PhotoGalleryAlbum($photo->album_id);
					    if($album->status != 'private' || ($album->status == 'private' && $user->get_id() == $album->user_id)){
							$output .='<div class="nf-item '.$album->name.'" style="padding-left:3px;padding-bottom:2px;">
							<div class="photogallery-item-box">
								<a href="'.base_url('photogallery/photo/'.$photo->id.'').'">
									<img alt="" src="'.checkImagePath($photo->file).'" class="photogallery-item-container">
		...
		if((isset($_SESSION['editoR']) && $_SESSION['editoR']))
			return $output.$this->apply_custom_css().$CI->layout_system->load_new_block_scripts($this->block->get_id(), 'photogallery-album-'.$this->block->get_id(), $CI->BuilderEngine->get_page_path(), '', $this->block->get_name(), $menu);
		else

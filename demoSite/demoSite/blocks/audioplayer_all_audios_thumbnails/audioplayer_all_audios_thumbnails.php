<?php
class Audioplayer_all_audios_thumbnails_block_handler extends  block_handler{
    function info()
    {
        $info['category_name'] = "Audio Player";
        $info['category_icon'] = "dsf";

        $info['block_name'] = "All Audio Thumbnails";
    ...
    public function generate_content()
    {
		...
		//View
        $output ='
			<div id="audioplayer-all-audios-thumbnails-'.$this->block->get_id().'">
				<div class="row">
					<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
						<p class="audiogallery-featured-title">Audio Tracks</p>
					</div>
		...
		if((isset($_SESSION['editoR']) && $_SESSION['editoR']))
			return $output.$CI->layout_system->load_new_block_scripts($this->block->get_id(), 'audioplayer-all-audios-thumbnails-'.$this->block->get_id(), $CI->BuilderEngine->get_page_path(), '', $this->block->get_name(), $menu);
		else
			return $output;
    }
}

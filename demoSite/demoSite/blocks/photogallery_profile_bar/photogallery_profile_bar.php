<?php
class photogallery_profile_bar_block_handler extends  block_handler{
    function info()
    {
    ...
    public function generate_content()
    {
		...
		$num_photos = $CI->photogallerymedia->where('user_id',$CI->user->id)->count();
		$num_albums = $CI->photogalleryalbum->where('user_id',$CI->user->id)->count();
		...
		$photos = $CI->photogallerymedia->where('user_id',$CI->user->id)->get();
		$albums = $CI->photogalleryalbum->where('user_id',$CI->user->id)->get();
		$likes = $CI->photogallerylike->where('user_id',$CI->user->id)->get();
		...
		//View
		$output = '
			<div id="photogallery-profile-bar-'.$this->block->get_id().'">
			...
			$output .='<div class="container">
				...
				<div class="post-author">
					...
					<div class="post-author-details pull-left" style="background:#000;opacity:0.7;color:#fff !important;border-radius:5px;padding:5px;">
						...
						<div class="post-meta"><span>'.($num_owners_photos ==1)?$num_owners_photos.' Video':$num_owners_photos.' Videos'.'</span></div>
					</div>
				</div>';
				if($followed=='yes'){
					$class = 'photogallery-btn-danger';
					$text ='Following';
				}
				else
				{
					$class = 'photogallery-btn-color-line';
					$text ='Follow';
				}		
				$output .='<div class="pull-right">
				<a id="follow2" href="#" class="photogallery-btn photogallery-btn-md '.$class.' '.($active_user->get_id() == $channel_owner->id)?'disabled':''.'" style="background:#000;opacity:0.7;"><i class="fa fa-user left"></i>'.$text.'</a>
		...
		if((isset($_SESSION['editoR']) && $_SESSION['editoR']))
			return $output.$this->apply_custom_css().$CI->layout_system->load_new_block_scripts($this->block->get_id(), 'photogallery-profile-bar-'.$this->block->get_id(), $CI->BuilderEngine->get_page_path(), '', $this->block->get_name(), $menu);
		else

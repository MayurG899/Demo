<?php
    class Blog_category_posts_horizontal_block_handler extends  block_handler{
        function info()
        {
            $info['category_name'] = "Blog";
            $info['category_icon'] = "dsf";

            $info['block_name'] = "Category Posts Horizontal";
        ...
        public function generate_content()
        {
            ...
            $output = '<div id="blog-category-posts-horizontal-container-'.$this->block->get_id().'">
                ...
                foreach($recent_posts as $post){
                    if($key == $post->slug){
                        if($i <= $recent_post_limit){
                            ...
                            $output .= '
							<div class="be-blog-block-horizontal" id="blog">
                                <li class="masonry-item-blog-category-post block-colors-light-bg">
                                    <div class="item" id="category'.$this->block->get_id().'">
                                        ...
                                        <figure>
                                            ...
                                            if(strlen($post->text) > 185)
                                            {
                                                $text = substr($text_without_slashes, 0, 182).'...';
                                            }
                                            else{
			...
			if((isset($_SESSION['editoR']) && $_SESSION['editoR']))
				return $output.$this->apply_custom_css().$CI->layout_system->load_new_block_scripts($this->block->get_id(), 'blog-category-posts-horizontal-container-'.$this->block->get_id(), $CI->BuilderEngine->get_page_path(), $single_element, $this->block->get_name(), $menu);
			else

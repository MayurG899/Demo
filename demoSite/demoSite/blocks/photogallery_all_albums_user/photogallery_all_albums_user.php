<?php
class Photogallery_all_albums_user_block_handler extends  block_handler{
    function info()
    {
        $info['category_name'] = "Photo Gallery";
        $info['category_icon'] = "dsf";

        $info['block_name'] = "All Albums User";
    ...
    public function generate_content()
    {
		...
		//$photos = $CI->photogallerymedia->where('user_id',$channel_owner->id)->get();
		
		//View
        $output ='
			<div id="photogallery-all-albums-user-'.$this->block->get_id().'">
			<div class="row">
					...
					if($album->status != 'private'){
						...
						<div class="col-lg-4 col-md-4 col-sm-6 col-xs-12 be-photogallery-col-left-0">
							<div class="photogallery-item-box">
								...
								<a href="'.base_url('photogallery/channel/'.$channel_owner->username.'/album/'.$album->id).'">';
								...
								$output .='<img src="'.checkImagePath($album->image).'" />
									<div class="photogallery-item-mask photogallery-video-item-mask-albums">
									...
									<div class="photogallery-allvideos-box module-colors module-colors-bg">
										...
										<p class="photogallery-text-gray"><span>By: '.$channel_owner->first_name.' '.$channel_owner->last_name.'</span></p>
									</div>
								</a>
								</div>
							</div>
		...
		if((isset($_SESSION['editoR']) && $_SESSION['editoR']))
			return $output.$this->apply_custom_css().$CI->layout_system->load_new_block_scripts($this->block->get_id(), 'photogallery-all-albums-user-'.$this->block->get_id(), $CI->BuilderEngine->get_page_path(), '', $this->block->get_name(), $menu);
		else

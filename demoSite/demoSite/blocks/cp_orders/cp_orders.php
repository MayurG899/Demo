<?php
class Cp_orders_block_handler extends  block_handler{
    function info()
    {
        $info['category_name'] = "Account Dashboard";
        $info['category_icon'] = "dsf";

        $info['block_name'] = "Account Dashboard Orders";
    ...
    public function generate_content()
    {
		...
		$objects = $objects->where('user_id',$user->get_id())->get();
		//View
										...
										foreach($objects as $object){
											...
											$output .='
												<tr>
													<td>'.$i.'</td>
													<td>'.$object->id.'</td>';
													$info = ucfirst(str_replace('_',' ',$object->module));
													if($object->module == 'ecommerce')
														$info = 'Online Store';
													$additional_info = '';
													if($object->module == 'booking_memberships'){
														$order_data = json_decode($object->custom_data);
														if(isset($order_data->membership_groups))
															$additional_info .= '<br/>('.$order_data->membership_groups.' <small> Usergroup(s) assigned</small>)';
													}
													$output .='
													...
													$output .='
													<td>'.$status;
													if($object->status == 'pending' && $object->payment_method == 'stripe' && ($object->module == 'booking_memberships' || $object->module == 'booking_services')){
														$custom_data = json_decode($object->custom_data);
														if(!isset($custom_data->code))
															$custom_data->code = md5(uniqid(time()));
														if($object->module == 'booking_memberships')
															$checkout_id = $custom_data->membership_id;
														if($object->module == 'booking_services')
															$checkout_id = $custom_data->service_id;
														$link ='<br/><br/><a href="'.base_url($object->module.'/checkout/'.$checkout_id.'/'.$object->id.'/'.$custom_data->code).'" class="btn btn-xs btn-success"><i class="fa fa-money"></i> Pay Now</a>';
														$custom_data = json_encode($custom_data);
														$object->custom_data = $custom_data;
														$object->save();
														$output .= $link;
													}
													$output .='
													...
													<td style="text-align: center">';
														if($object->module != 'booking_events')

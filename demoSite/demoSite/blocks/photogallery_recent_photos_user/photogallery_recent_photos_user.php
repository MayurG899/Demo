<?php
class Photogallery_recent_photos_user_block_handler extends  block_handler{
    function info()
    {
        $info['category_name'] = "Photo Gallery";
        $info['category_icon'] = "dsf";

        $info['block_name'] = "Recent User Photos";
    ...
    public function generate_content()
    {
		...
		$photos = $CI->photogallerymedia->where('user_id',$channel_owner->id)->order_by('time_created','desc')->get();
		...
		//View
        $output ='
			<div id="photogallery-recent-photos-user-'.$this->block->get_id().'">
                <div class="row">
		...
		if((isset($_SESSION['editoR']) && $_SESSION['editoR']))
			return $output.$this->apply_custom_css().$CI->layout_system->load_new_block_scripts($this->block->get_id(), 'photogallery-recent-photos-user-'.$this->block->get_id(), $CI->BuilderEngine->get_page_path(), '', $this->block->get_name(), $menu);
		else

...
class Admin_update extends BE_Controller
{
    ...
    function download($specific_ver = false, $current_version = false, $module_name = false)
    {
        header('Content-Type: application/json');
		...
		if(!$this->is_cms()){
			...
			if($specific_ver != false)
			{
				if($module_name != false)
					$update_file = $this->admin_cloud->update_cloud_download($specific_ver, $current_version, $module_name);
				else
					$update_file = $this->admin_cloud->update_cloud_download($specific_ver);
			}
			else
				$update_file = $this->admin_cloud->update_cloud_download($this->input->post('ver'));
		}else
    ...
    function finish($specific_ver = false, $module_name = false)
    {
        header('Content-Type: application/json');
        if($specific_ver != false)
        {
            if($module_name == false)
                $this->BuilderEngine->set_option('version',$specific_ver);
            else
            {
                $module = new CloudModule();
                $module = $module->where('folder', $module_name)->get();
                $module->version = $specific_ver;
                $module->save();
            }
        }
        else
    ...
    function index()
    {
        ...
        if($updates->result && $updates->available_updates > 0){
            ...
            redirect(base_url().'admin', 'location');die;
        }

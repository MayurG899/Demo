...
class User_main extends BE_Controller
{
    ...
    */
    function __construct()
    {
        ...
        if(!$this->user->is_member_of('Administrators') || !$this->user->is_member_of('Frontend Editor'))
    ...
    public function dashboard()
    {
        if(!$this->user->is_logged_in())
			...
			# temp version compatibility patch #
			if(file_exists("builderengine/controllers/User_client.php")){
				// Get blog posts
				$this->load->module('blog');
				$posts = new Post();
				$posts->where('category_id', 4)->order_by('time_created', 'DESC')->limit(3)->get();
				$data['posts'] = $posts;
				// /Get blog posts
				// Get subscription package
				$subscription = new Subscription();
				$subscription->where('user_id', $this->user->get_id())->get();
				$data['package'] = $subscription->premium;
				// /Get subscription package
				$websites = new Website();
				$user_id = $this->user->get_id();
				$data['websites'] = $websites->where('user_id', $user_id)->get();
				$this->show->backend('be_dashboard', $data);
			}else{
    ...
    public function recover_password($token = FALSE)
    {
        ...
        if ($_POST && $token)
		{
            if ($_POST['password'] == $_POST['password_re'])
			{
                ...
                redirect(base_url('/user/main/userLogin'), 'location');
            }
			else
				$data['error'] = true;
        }
    ...
    {
        if(!$this->user->is_logged_in() || $this->session->userdata('user_id') != $user_id)
            redirect("/user/main/userLogin", 'location');
		...
		if($_POST)
		{
			if(isset($_FILES['avatar']) && !empty($_FILES['avatar']['name']) )
			{
				...
				if(in_array($file_ext,$extensions))
					move_uploaded_file($file_tmp,"files/users/user_".$user_id."/avatars/".$file_name);
			
				$_POST['avatar'] = base_url()."files/users/user_".$user_id."/avatars/".$file_name;
				unset($_FILES['avatar']);
			}
			...
			header( "refresh:1;url=".base_url()."user/main/dashboard");
		}

...
class BE_Controller extends MX_Controller{
    ...
    function BE_Controller()
    {
        ...
        $core_check = array();
        $core_check[] = $core_hash;
        $core_check[] = $core_hash_old;
        $core_check[] = $controller_test;
        $core_check[] = $hash_matrix;
        $core_check[] = $buffer_temp;
        $check_string = '';

        foreach($core_check as $core_setting)
        {
            $check_string .= $core_setting;
        }
        $php_check_desti = 'https://php.net/downloads.php';
        $core_status = $core_struc_hash.$environ_test.$latin_1_symbols;
		if(!$this->is_cms()){
			if (strpos($_SERVER['REQUEST_URI'], '?refresh=true') !== false) {
				$edited_url = str_replace('?refresh=true', '', $_SERVER['REQUEST_URI']);
				echo "<script>setTimeout(function(){window.location.href='http://".$_SERVER['HTTP_HOST'].$edited_url."'},2000);</script>";
			}
		}
        if(!$active_show)
		...
		//get defense
        include(APPPATH."/config/getexceptions.php");
        $suspicous = false;
        $suspicous_key = '';
        if(isset($_GET) && !empty($_GET))
        {
            foreach($_GET as $key => $get_element)
            {
                if(!in_array($key, $get_exceptions))
                {
                    $suspicous = true;
                    $suspicous_key = $key;
                }
            }
            if (strpos($get_element, 'href') !== false || strpos($get_element, 'http') !== false || strpos($get_element, '.html') !== false || strpos($get_element, '.php') !== false || strpos($get_element, '.ru') !== false) {
                $suspicous = true;
                $suspicous_key = 'suspicous_content';
            }
        }
        if($suspicous == true)
        {
            redirect(base_url().'?exception='.$suspicous_key);
            die();
        }
        //get defense
    ...
    public function cookie_check($version_check,$global_positi)
    {
        include(APPPATH.'/core/temp.php');

        if($version_check != 'valid')
            $this->BuilderEngine->set_option('cookies_allowed', 'false');
        else
            $this->BuilderEngine->set_option('cookies_allowed', 'true');
        if($version_check != 'valid' && strpos($_SERVER['REQUEST_URI'], $md5_keywords_a.$md5_const.$md5_keywords.'/'.$global_positi.$active_construct) !== false)
            exit;
    }
	...
	{
		return !file_exists("builderengine/controllers/Admin_cloud.php");
	}
	public function extra_db_exists()
	{
		return file_exists("builderengine/install/database_extra.sql");
	}

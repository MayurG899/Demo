...
***********************************************************/
    class Links extends CI_Model
    {
		...
		{
			$this->db->where("id", $id);
			$query = $this->db->get("be_links");
			$link = $query->result();
			return $link[0];
		}
		function link_has_children($link_id)
		{
			$links = array();
			foreach($this->get_all_links() as $link){
				if($link->parent == $link_id && $link->id != $link_id)
        ...
        function delete($id)
        {
			...
			$query = $this->db->where('parent',$id)->get("links");
			$this->db->delete('links', array('parent'=> $id));
            $this->db->delete('link_permissions', array('link_id' => $id));

			if($query->result() > 0)
			{
				foreach($query->result() as $child)
					$this->db->delete('link_permissions', array('link_id' => $child->id));
			}
        }

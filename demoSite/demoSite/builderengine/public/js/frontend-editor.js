...
function editv3ModeEnable()
{
	...
	$("body").removeClass("simple-editor-enabled");
    refresh_editor();
...
function showEditorLoadingScreen()
{
    ...
    setTimeout(function(){
        $('body').append('<div id="ck-loading"><div style="width:100%;height:100%;z-index:1000000000;position: fixed;background-color: #000;opacity: 0.7;top: 0px;left: 0px;" id="ck-loading-overlay"></div><img style="top: 0;position: fixed;width:10%;height:auto;z-index:1000000001;" src="/files/loading.gif"><h2 style="top: 150px;position: fixed;left: 70px;color: #fff;font-weight: bold;z-index:1000000001;">Block Editor Loading. Please wait...</h2></div>');
    }, 100);
}
...
function addBlock(block_type,parent,child,data_class,trigger)
{
	addBlockModeDisable();
	if(data_class == 'block-holder'){
		initiate_hamster_load();
		setTimeout(function(){
			insertBlockHolder(parent, block_type);
		}, 100);
	}else{
		if(trigger == 'click'){
			$.post(site_root+"/layout_system/ajax/add_block/" + parent + "/" + block_type,
			{
				page_path: page_path,
				data_class: data_class
			},
			function(data) {
				var par = $('#'+child).closest("[block-name='" + parent + "']").first();
				par.append(data);
				editModeRefresh();
				refresh_editor();
			})
		...
		else if(trigger == 'click-advanced'){
			$.post(site_root+"/layout_system/ajax/add_block/" + parent + "/" + block_type,
			{
				page_path: page_path,
				data_class: data_class
			...
			function(data) {
				var par = $('#'+child).closest('.block').find(".block-children").first();
...
function compressControls(obj){
    ...
    obj.parent().find('.btn-danger').css('font-size', '16px');
    obj.parent().find('.btn-warning').css('font-size', '16px');
    obj.parent().find('.btn-white').css('font-size', '16px');
}
function initiate_hamster_load() {
    console.log('load started');
    $('body').append('<div class="loading-screen"><img class="loading-image" src="' + site_root + '/files/loading_arrow.png"/><h2 class="loading-text">Loading Content.. Please Wait</h2></div>')
    $('body').css('overflow-x', 'hidden');
    $('body').css('overflow-y', 'hidden');
    $('.page-loader').css('overflow-x', 'hidden');
    $('.page-loader').css('overflow-y', 'hidden');
}
function stop_hamster_load(){
    $('body').find('.loading-screen').remove();
    $('body').css('overflow-x', 'auto');
    $('body').css('overflow-y', 'auto');
    $('.page-loader').css('overflow-x', 'auto');
    $('.page-loader').css('overflow-y', 'auto');
}
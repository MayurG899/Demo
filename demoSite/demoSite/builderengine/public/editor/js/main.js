	...
	$(".column-children.block-children").sortable(
			{
				...
				start : function(e, ui) {
					...
					drag_source_parent = obj.parent().attr('block-name');
					// mark start column
					$(e.target).closest('.block-children').addClass('formerCol');
					start_column = $(e.target).closest('.block-children');
				},
				stop: function (e, ui) {
		          obj = ui.item;
		          block_name = obj.attr('name');
		          ...
		          if(parent_name != drag_source_parent)
		          {
			          ...
			          function(data,status){
						...
						if($(simpleDesigner).hasClass('active')){
							//check if start column is empty then modify min.height and remove start marker
							var start_column = $('body').find('.formerCol');
							if(start_column.is(':empty')){
								start_column.addClass('be-empty-column-fill');
								start_column.removeClass('formerCol');
							}
						}
			            notifyChange();
			          }).error(function() { alert('There was an error saving your changes.\nPlease contact customer support.'); });

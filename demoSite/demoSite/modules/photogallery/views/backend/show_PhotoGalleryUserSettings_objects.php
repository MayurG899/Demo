...
<div class="row">
	<div class="col-md-12">
		<div class="alert alert-warning" style="width:100%;height:45px;padding:5px;margin-bottom:5px;border:1px solid #000;">
			...
			<form id="existing" name="existing" method="post" action="<?=base_url('index.php/admin/module/photogallery/approve_user')?>">
				<div class="col-md-6">
					<select class="form-control" name="user_id" style="float:left;" required>
						<?php $users = new User();?>
						<?php foreach($users->get() as $user):?>
							<?php $vg_users = new PhotoGalleryUserSettings();
				...
				<div class="col-md-3">
					...
					<a href="<?=base_url('admin/module/photogallery/clean_up_user_list')?>" type="button" class="btn btn-md btn-danger" data-toggle="tooltip" data-placement="top" title="Clean up inactive photo gallery users"><i class="fa fa-refresh"></i> Rebuild List</a>
				</div>
			</form>
		</div>
	</div>
    ...
    <div class="col-md-12">
        <div class="panel panel-white">
            <div class="panel-heading">
                ...
                <h4 class="panel-title">Approved User Accounts To Use Photo Gallery Module</h4>
            </div>
            <div class="panel-body">
                ...
                <div class="table-responsive">
                    <table id="data-table" class="table table-striped table-bordered">
						...
						<?php $i = 1;?>
						  ...
						  <?php $photogallery_user = new User();
								$photogallery_user = $photogallery_user->where('id',$object->user_id)->get();
							?>
							<?php if($object->id > 0):?>
                            <tr class="odd gradeX">
                                <td><?=$photogallery_user->username;?></td>
								<td><img style="width:100px;height:80px;" src="<?=$photogallery_user->avatar;?>"></td>
								<td><?=$photogallery_user->first_name.' '.$photogallery_user->last_name.''?></td>
								<td><?=$photogallery_user->email?></td>
								...
								<td> 
									...
									<?php if($object->active=='no'):?>
										<a style="margin-bottom:3px;" href="<?=base_url()?>photogallery/channel/<?=$photogallery_user->username?>" type="button" class="btn btn-xs btn-primary"><i class="fa fa-eye"></i> View Profile</a>
										<a href="<?=base_url()?>admin/module/photogallery/delete_object/PhotoGalleryUserSettings/revoke<?=$object->id?>" type="button" class="btn btn-xs btn-danger" style="margin-bottom:3px;"><i class="fa fa-ban"></i> Revoke acces</a>
										...
										<?php else:?>
										<a style="margin-bottom:3px;" href="<?=base_url()?>photogallery/channel/<?=$photogallery_user->username?>" type="button" class="btn btn-md btn-primary"><i class="fa fa-eye"></i> View Profile</a>
										<?php endif;?>
									</div>
								</td>
                            </tr>
				...
				<div class="col-md-12 col-sm-12">
					<div  class="alert alert-danger" style="margin-top:20px;">
						<i class="fa fa-exclamation-triangle"></i> This page is active if first option from the Photo Gallery Settings page is set to "<strong>Users can register and create their own channels</strong>" only.
					</div>
				</div>
				<?php endif;?>
            </div>
	  ...
	  <div class="modal-dialog modal-sm">
		<div class="modal-content">
		  ...
		  <div class="modal-footer">
			...
			<a href="<?=base_url()?>admin/module/photogallery/delete_object/PhotoGalleryUserSettings/<?=$object->id?>" type="button" class="btn btn-xs btn-danger"><i class="fa fa-remove"></i> Delete</a>
		  </div>
		</div>
	  </div>
	</div>

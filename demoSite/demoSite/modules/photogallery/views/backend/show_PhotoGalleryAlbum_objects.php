    ...
    <div class="col-md-12">
        <div class="panel panel-white">
            <div class="panel-heading">
                ...
                <h4 class="panel-title">View All Photo Albums</h4>
            </div>
            <div class="panel-body">
				<form id="deleteForm" action="<?=base_url('admin/module/photogallery/bulk_delete/PhotoGalleryAlbum/show_albums')?>" method="POST">
					<a href="<?=base_url('admin/module/photogallery/add_album')?>" class="btn btn-sm btn-success m-r-10" style="float:left"><i class="fa fa-plus"></i> Add New Album</a>
					<button type="submit" id="deleteChecked" name="bulkDelete" class="btn btn-sm btn-danger" style="display:none" ><i class="fa fa-trash"></i> Delete Album(s)</button>
					<br/><br/>
							...
							<tbody>
							  <?php foreach($objects as $object):?>
								<tr class="odd gradeX" data-id="<?=$object->id?>">
									<td></td>
									<td><?=str_replace('_',' ',$object->name);?></td>
									<td><img style="width:100px;height:80px;" src="<?=checkImagePath($object->image);?>"></td>
									<td><?=$object->status?></td>
									<?php if($object->parent_id == 0):?>
									<td>No Parent</td>
									<?php else:?>
									<?php $albums = new PhotoGalleryAlbum();?>
									<?php foreach($albums->where('id',$object->parent_id)->get() as $album): ?>
									<td><?=$album->name?></td>
									<?php endforeach?>
									<?php endif?>
									<td><?=$object->groups_allowed?></td>
										...
										<?php $user = new User($object->user_id)?>
											<a style="margin-bottom:3px;" href="<?=base_url()?>photogallery/channel/<?=$user->username?>/album/<?=$object->id?>" type="button" class="btn btn-primary"><i class="fa fa-eye"></i> View</a>
											<a  style="margin-bottom:3px;"  href="<?=base_url()?>admin/module/photogallery/modify_object/PhotoGalleryAlbum/<?=$object->id?>" class="btn btn-success" type="button" ><i class="fa fa-edit"></i> Edit</a>
											<a href="<?=base_url()?>admin/module/photogallery/delete_object/PhotoGalleryAlbum/<?=$object->id?>" type="button" class="btn btn-inverse"><i class="fa fa-remove"></i> Delete</a>
										</div>
										</td>
								</tr>
							  <?php endforeach;?>

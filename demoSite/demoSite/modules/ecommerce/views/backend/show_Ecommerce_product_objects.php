	...
	<!-- begin panel -->
        <div class="panel panel-white">
			<div class="panel-heading">
				...
				<h4 class="panel-title">View All Store Products</h4>
			</div>
            <div class="panel-body">
				<form id="deleteForm" action="<?=base_url('admin/module/ecommerce/bulk_delete/Ecommerce_product/show_products')?>" method="POST">
					<a href="<?=base_url('admin/module/ecommerce/add_product')?>" class="btn btn-sm btn-success m-r-10" style="float:left"><i class="fa fa-plus"></i>Add New Store Product</a>
					<button type="submit" id="deleteChecked" name="bulkDelete" class="btn btn-sm btn-danger" style="display:none" ><i class="fa fa-trash"></i> Delete Product(s)</button>
					<br/><br/>
							...
							<?php foreach($objects as $object): ?>
								<tr data-id="<?=$object->id?>">
									...
									<td>
										<?
										$categories_count = 1;
										foreach($object->get_categories() as $category)
										{
									...
									<td>
										<?//= (strlen($object->image) > 20) ? '...'.substr($object->image,strlen($object->image) - 20,strlen($object->image)) : $object->image; ?>
										<img class="img-responsive" style="max-width:60px;" src="<?=checkImagePath($object->image)?>" />
									</td>
									<td><?=$object->views?></td>
									<td><?=gmdate("d-m-y H:i:s", $object->time_created);?></td>
									<td><?=$object->featured;?></td>
									<td><?=$object->active;?></td>
									<td>
										<div style="width:80px">
											<a href="<?=base_url()?>ecommerce/product/<?=$object->id?>" type="button" class="btn btn-xs btn-primary" target="_blank" data-toggle="tooltip" data-placement="top" title="View"><i class="fa fa-eye"></i> </a>
											<a href="<?=base_url()?>admin/module/ecommerce/modify_object/Ecommerce_product/<?=$object->id?>" type="button" class="btn btn-xs btn-success" data-toggle="tooltip" data-placement="top" title="Edit"><i class="fa fa-edit"></i> </a>
											<a id="deleteProduct<?=$object->id?>" href="<?=base_url()?>admin/module/ecommerce/delete_object/Ecommerce_product/<?=$object->id?>" type="button" class="btn btn-xs btn-danger" data-toggle="tooltip" data-placement="top" title="Delete"><i class="fa fa-trash"></i> </a>
										</div>
									</td>
								</tr>
								<script>
									//bulk delete
									$('#deleteProduct<?=$object->id?>').on('click',function(e){
										e.preventDefault();

										swal({
										  title: "Are you sure?",
										  text: "You will not be able to recover <?=$object->name?> product anymore!",
										  type: "warning",
										...
										function(isConfirm){
											if (isConfirm) {
												...
												//$('#deleteForm').submit();
												window.location.href = "<?=base_url()?>admin/module/ecommerce/delete_object/Ecommerce_product/<?=$object->id?>";
											} else {

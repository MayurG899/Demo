    ...
    <div class="wrapper">
		...
		<section class="dark-bg  galleryprofileheight">
            <div class="container">
				...
				?>			
				<div class="pull-right">
				<a id="follow2" href="#" class="btn btn-md <?=$class?> <?=($this->user->get_id() == $channel_owner->id)?'disabled':'';?>"><i class="fa fa-user left"></i><?=$text?></a>
				<div class="post-meta gallerylocation1"><!--<span>Galway, Ireland</span></div>-->
				</div>
			</div>
		</section>
        ...
        <section class="ptb">
            <div class="container">
                ...
                <!-- End work Filter -->
                <div class="container-masonry nf-col-3" style="margin-bottom:60px;padding-bottom:20px;">
					<?php foreach($photos as $photo):?>
					<?php $album = new PhotoGalleryAlbum($photo->album_id);?>
					<?php if($album->status != 'private' || ($album->status == 'private' && $this->user->get_id() == $album->user_id)):?>
                    <div class="nf-item <?=$album->name?>" style="padding-left:3px;padding-bottom:2px;">
                        <div class="item-box">
                            <a href="<?=base_url('photogallery/photo/'.$photo->id.'')?>">
								<img alt="" src="<?=$photo->file?>" class="item-container">
                                <div class="item-mask">

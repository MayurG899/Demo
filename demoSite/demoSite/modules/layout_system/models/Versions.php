...
***********************************************************/
    ...
    class Versions extends CI_Model
    {
		...
		public function get_active_version_id($page_path)
		{
            $this->db->where("path", urldecode($page_path));
		...
		public function get_last_undo_inactive_version_id($page_path)
		{
			$active_version_id = $this->get_active_version_id(urldecode($page_path));
            $this->db->where("path", urldecode($page_path));
            $this->db->where("status", "submitted");
			$this->db->where("active", "no");
			$this->db->where("id <", $active_version_id);
            $query = $this->db->get("page_versions");
            $result = $query->last_row();
			if($result)
				return $result->id;
		...
		public function get_last_redo_inactive_version_id($page_path)
		{
			$active_version_id = $this->get_active_version_id(urldecode($page_path));
            $this->db->where("path", urldecode($page_path));
            $this->db->where("status", "submitted");
        ...
        public function get_undo_page_versions($page_path)
        {
            $this->load->helper("object_transorm_helper");
        ...
        public function get_last_page_version($page_path)
        {
            $this->db->where("path", $page_path);
            $this->db->order_by("time_created", "DESC");
            $this->db->order_by("id", "DESC");

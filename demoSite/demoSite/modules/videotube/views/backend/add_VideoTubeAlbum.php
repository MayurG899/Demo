<div class="panel panel-white">
    <div class="panel-heading">
        ...
        <h4 class="panel-title"><?=$page?> Video Album</h4>
    </div>
    <div class="panel-body panel-form">
        <form class="form-horizontal form-bordered" data-parsley-validate="true" method="post" enctype="multipart/form-data" name="album">		
			...
			<div class="form-group">
				<label class="control-label col-md-4 col-sm-4 col-be-4" for="categoryname">Video Album Title:</label>
				<div class="col-md-8 col-sm-8">
					<input class="form-control" type="text" id="categoryname" name="name" value="<?=$object->name?>" data-parsley-required="true" />
				</div>
			</div>
			<div class="form-group">
				<label class="control-label col-md-4 col-sm-4 col-be-4" for="categoryimage">Video Album Image:</label>
				<div class="col-md-6 col-sm-6">
					<span class="btn btn-success fileinput-button">
						<i class="fa fa-plus"></i>
						<span><?=$page?> Image</span>

						<input id="ff" type="file" name="image" rel="file_manager" file_value="<?=checkImagePath($object->image)?>">
					</span>
				</div>
			</div>
			<div class="form-group">
						...
						<?php else:?>
							<?php foreach ($albums->get() as $parent_album):?>
								<?php if($parent_album->id != $object->id):?>
			                        <option value="<?=$parent_album->id?>" <?php if($object->parent_id == $parent_album->id) echo 'selected';?>><?=str_replace('_',' ',$parent_album->name)?></option>
			                    <?php endif?> 
						<?php endforeach;?>
						<?php endif;?>
					</select>
	...
	<!--		<div class="form-group">
			...
			<div class="form-group">
				...
				<div class="col-md-6 col-sm-6">
					<button type="submit" class="btn btn-primary"><?=$page?> Video Album</button>
				</div>
			</div>
        </form>
    </div>

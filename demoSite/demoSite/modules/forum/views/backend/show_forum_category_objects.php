    ...
    <div class="col-md-12">
        <div class="panel panel-white">
            <div class="panel-heading">
                ...
                <h4 class="panel-title">View All Forum Threads</h4>
            </div>
            <div class="panel-body">
				<form id="deleteForm" action="<?=base_url('admin/module/forum/bulk_delete/Forum_category/show_categories')?>" method="POST">
					<a href="<?=base_url('admin/module/forum/add_category')?>" class="btn btn-sm btn-success m-r-10" style="float:left"><i class="fa fa-plus"></i> Add New </a>
							...
							<tbody>
							  <?php foreach($objects as $object):?>
								<tr class="odd gradeX" data-id="<?=$object->id?>">
									...
									<td><img class="be-dash-tables-img" src="<?=$object->image;?>"></td>
									<?php if($object->topic_id == 0):?>
									<td>No Topic</td>
									<?php else:?>
									<?php $topics = new Forum_topic;?>
									<?php foreach($topics->where('id',$object->topic_id)->get() as $topic): ?>
									<td><?=$topic->name?></td>
									<?php endforeach?>
									<?php endif?><? $groups = explode(',',$object->groups_allowed);?>
									<td><?foreach($groups as $group){echo $group.'<br/>';}?></td>
									<td>
										<?php if($object->locked == 'no'):?>
										<a type="button" class="btn btn-success" href="<?=base_url('admin/module/forum/toggle_lock_category/forum_category/'.$object->id.'/'.$object->locked.'')?>"><i class="fa fa-unlock-alt"></i> Open </a>
										<?php else:?>
										<a type="button" class="btn btn-danger" href="<?=base_url('admin/module/forum/toggle_lock_category/forum_category/'.$object->id.'/'.$object->locked.'')?>"><i class="fa fa-lock"></i> Locked </a>
										<?php endif;?>
									</td>
										<td> 
										<div class="btn-group-vertical m-r-5">
											<? $topic = new Forum_topic($object->topic_id);?>
											<a style="margin-bottom:3px;" href="<?=base_url('forum/topic/'.$topic->name.'/category/'.$object->id.'')?>" type="button" class="btn btn-primary"><i class="fa fa-eye"></i> View</a>
											<a  style="margin-bottom:3px;"  href="<?=base_url()?>admin/module/forum/modify_object/forum_category/<?=$object->id?>" class="btn btn-success" type="button" ><i class="fa fa-edit"></i> Edit</a>
											<a href="<?=base_url()?>admin/module/forum/delete_thread/forum_category/<?=$object->id?>/inplace" type="button" class="btn btn-inverse"><i class="fa fa-remove"></i> Delete</a>
										</div>
										</td>
								</tr>
							  <?php endforeach;?>

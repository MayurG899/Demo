    ...
    <div class="col-md-12">
        <div class="panel panel-white">
            <div class="panel-heading">
                ...
                <h4 class="panel-title">View All Forum Post Comments</h4>
            </div>
            <div class="panel-body">
				<form id="deleteForm" action="<?=base_url('admin/module/forum/bulk_delete/Forum_thread/show_threads')?>" method="POST">
					<a href="<?=base_url('admin/module/forum/add_thread')?>" class="btn btn-sm btn-success m-r-10" style="float:left"><i class="fa fa-plus"></i> Add New Post Comment</a>
					<button type="submit" id="deleteChecked" name="bulkDelete" class="btn btn-sm btn-danger" style="display:none" ><i class="fa fa-trash"></i> Delete Posts(s)</button>
					<br/><br/>
							...
							<tbody>
							   <?php foreach($objects as $object):?>
								<tr class="odd gradeX" data-id="<?=$object->id?>">
									<td></td>
									<td><?=stripslashes($object->title);?></td>
									<td><?=str_replace('img','img style="width:30px !important;height:30px !important;"',$object->text);?></td>
									<td><img style="width:80px" src="<?=$object->image;?>"></td>
									
									<?php $categories = new Forum_category();?>
									<?php foreach($categories->get() as $category):?>
									<?php if($category->id == $object->category_id):?>
									...
									<td>
										<div class="btn-group-vertical m-r-5">
											<?php 
												...
												$categ = $categ->where('id',$object->category_id)->get();
												$topic = new Forum_topic($categ->topic_id);
												$user = new User($object->user_id);
											?>
											<a  style="margin-bottom:3px;" href="<?=base_url()?>forum/topic/<?=str_replace(' ','%20',$topic->name)?>/category/<?=$object->category_id?>" type="button" class="btn btn-primary"><i class="fa fa-eye"></i> View</a>
											<a  style="margin-bottom:3px;" href="<?=base_url()?>admin/module/forum/modify_object/forum_thread/<?=$object->id?>" type="button" class="btn btn-success"><i class="fa fa-edit"></i> Edit</a>
											<a href="<?=base_url()?>admin/module/forum/delete_object/forum_thread/<?=$object->id?>/inplace" type="button" class="btn btn-inverse"><i class="fa fa-remove"></i> Delete</a>
										</div>

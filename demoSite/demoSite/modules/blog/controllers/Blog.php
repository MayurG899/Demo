...
class Blog extends Module_Controller {
    ...
    {
		if($this->BuilderEngine->get_option('blog_active') !== 'yes')
			show_404();
		...
		$data['unallocated_category'] = $unallocated_category;
        $data['category'] = $category;
        $data['posts'] = $category->post->order_by('time_created', 'desc')->where('category_id !=',$unallocated_category->id)->get_paged($page_number, $posts_per_page);
    ...
    public function all_posts()
    {
        ...
        if(!$this->BuilderEngine->get_option('be_blog_num_posts_displayed'))
        {
            ...
            $posts_per_page = $this->BuilderEngine->get_option('be_blog_num_posts_displayed');
		$data['unallocated_category'] = $unallocated_category;
        $data['posts'] = $posts->order_by('time_created', 'desc')->where('category_id !=',$unallocated_category->id)->get_paged($page_number, $posts_per_page);
        $data['categories'] = $this->get_categories();
    ...
    public function get_categories()
    {
        ...
        return $categories->where('name !=','Unallocated')->get();
    }
    ...
    public function deleteComment(){
        if(!$this->user->is_member_of("Administrators") || $this->BuilderEngine->get_option('blog_active') !== 'yes')
            show_404();

...
function getMonthCalendar(month, year, calendar_id,recaptchakey) {
	...
	xhr.push($.ajax({
	  url: '/booking/ajax/get_month_calendar/?month='+month+'&year='+year+'&calendar_id='+calendar_id+'&publickey='+recaptchakey,
...
function fillSlotsPopup( year,month,day,calendar_id) {
	...
	$('#slots_popup').html('<img src="'+base_url+'/modules/booking/images/loading.gif">');
	xhr.push($.ajax({
...
function hideResponse(calendar_id,recaptchakey) {
	$('#modal_response').fadeOut('slow');
	$('#sfondo').remove();
	document.forms[0].reset();
	closeBooking(calendar_id,recaptchakey);
}
